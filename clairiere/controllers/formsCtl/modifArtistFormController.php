...
//}
if (isset($_POST['submit']) && !empty($_FILES['fileToUpload']['name']) && isset($_GET['id'])) {
...
// if everything is ok, try to upload file
        ...
        if (count($uploadError) == 0 && count($formError) == 0) {
            ...
            $bioModif = new biography();
            $bioModif->idUser = $_GET['id'];
            var_dump($bioModif);
            $modifiedBio = $bioModif->bioByIdUser();
            var_dump($modifiedBio);

            $fileToDelete = './img/profilePicture/' . $modifiedBio->profilePicture;
